...
// Donations
export async function saveDonation(data: Partial<Donation> & { txnid: string }): Promise<string> {
    const db = await getDb();

    // Use the txnid as the document ID, just like the webhook does
    const docRef = db.collection("donations").doc(data.txnid);

    // Set the data object with merge: true to avoid overwriting existing fields
